...
```{r}
tissue_plates = filter(plate_metadata, mouse.age == 3)[,c('plate.barcode','tissue','subtissue','mouse.sex')]
...
ribo.genes <- grep(pattern = "^Rp[sl][[:digit:]]", x = rownames(x = raw.data), value = TRUE)
...
```{r}
#save(tiss, file=paste0(save_dir,"/", "All_scaled_seurat_tiss.Robj"))
```
...
```
```{r}
table(tiss@ident, tiss@meta.data$tissue)

```


```{r}
TSNEPlot(object = tiss, do.return = TRUE, group.by = "tissue", pt.size = 0.1)
```

```{r}
TSNEPlot(object = tiss, do.return = TRUE, group.by = "subtissue", pt.size = 0.1)
```


```{r}
save(tiss, file=paste0(save_dir,"/", "All2_scaled_seurat_tiss.Robj"))
```
...
genes_to_check = c('Ins1', 'Pecam1', 'Pdgfra', 'Ptprc','Alb', 'Cd34', 'Cd8a', 'Cd3e')
...
```{r}
clust.markers <- FindMarkers(object = tiss, ident.1 = c(1,21), only.pos = TRUE, min.pct = 0.25, thresh.use = 0.25)
```
...
```{r}
TSNEPlot(object = tiss, do.return = TRUE, group.by = "mouse.sex", pt.size = 0.1)
```
