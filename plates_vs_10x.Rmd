...
```{r}
medians <- df %>% group_by(method, tissue, run) %>% summarize(median_Rn45s = median(percent.Rn45s), 
                                                                   median_ribo = median(percent.ribo),
                                                                   n_cells = n()) 
```

```{r}
medians %>% filter(tissue %in% common_tissues,) %>% ggplot(aes(x = median_Rn45s, y = median_ribo, color = tissue, shape = method)) + geom_point()  +
    scale_color_brewer(palette="Spectral") +
  ggtitle("Comparing Rn45s and ribo genes")
```
