...
Color the big TSNE by annotations that occur in more than one tissue.

```{r}
A <- function(x){
  if(x %in% repeated_annotations){
    x
  }else{
    NA
  }
...
```{r}
apply(anno$annotation, function(x){} if(x %in% repeated_annotatins){x}else NA})
