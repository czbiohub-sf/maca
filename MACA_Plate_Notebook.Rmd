...
```{r}
tissue_plates = filter(metadata, tissue == tissue_of_interest & mouse.age == 3)[,c('plate.barcode','tissue','subtissue','mouse.sex')]
...
tiss <- CreateSeuratObject(raw.data = data.no.erccs, project = tissue_of_interest, 
                    min.cells = 5, min.genes = 5)

if(length(tissue_counts) > 1){
  for (file in tissue_counts[2:length(tissue_counts)]){
